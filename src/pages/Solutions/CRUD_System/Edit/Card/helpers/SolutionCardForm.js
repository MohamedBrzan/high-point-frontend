...
import '../EditSolutionCard.css';

const SolutionCardForm = ({ handleClose }) => {
  ...
  const {
    ...
    isLoading,
    isSuccess,
  } = useGetSolutionByIdQuery(solutionCard._id && solutionCard._id);

  const [updateCard, { isLoading: Updating }] = useUpdateSolutionCardMutation();
  ...
  const [title, setTitle] = useState(solutionCard.title || '');

  const [titleAr, setTitleAr] = useState(solutionCard.title_ar || '');
  ...
  const [descriptionAr, setDescriptionAr] = useState(
    solutionCard.description_ar || ''
  );

  // Card Images

  const [cardImage, setCardImage] = useState(solutionCard.card_image || '');

  const [viewImage, setViewImage] = useState(solutionCard.view_image || '');

  // Card Image Text

  const [imageText, setImageText] = useState(solutionCard.image_text || '');

  const [imageTextAr, setImageTextAr] = useState(
    solutionCard.image_text_ar || ''
  );
  ...
  const updateCardData = async (e) => {
    try {
      ...
      await updateCard({ card_id: card._id, ...data })
        .then((response) => {
          dispatch(createSolutionCard(response.data));
  ...
  return (
    <section className='solution_schema_form'>
...
export default SolutionCardForm;
