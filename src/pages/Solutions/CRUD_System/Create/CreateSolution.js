...
import { useCreateSolutionSolutionMutation } from '../../../../store/apis/SolutionsSolutions/SolutionsSolutions';
import './Create.css';
...
const CreateServiceSolution = ({ tab_id }) => {
  ...
  const createNewSolution = async (e) => {
    try {
      ...
      if (!validator.isMongoId(tab_id)) {
        return toast.error(t('error.tab_id_error'), {
          position: 'top-center',
        });
      } else if (tab_id === '') {
        return toast.error(t('error.tab_id'), {
          position: 'top-center',
        });
      } else if (titleEn === '') {
      ...
      await createSolution({ tab_id, ...data }).then((response) => {
        if (response.data) {
          ...
          handleClose();
          document
            .querySelectorAll('.view-solutions .card_tab')
            .forEach((tab) => {
