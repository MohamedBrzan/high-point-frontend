...
import { useUpdateProductMutation } from '../../../store/apis/Product/Product';
import CustomSpinner from '../../../utils/CustomSpinner/CustomSpinner';
import UploadFile from '../../../functions/UploadFile';

const EditProductForm = ({ product, refetchProduct }) => {
  ...
  const updateProductData = async (e) => {
    try {
      ...
      e.preventDefault();
      await updateProduct({ product_id: product._id, ...data }).then(
        (response) => {
          if (response.data) {
  ...
  useEffect(() => {
    ...
    setSubTitleAr(product.sub_title_ar);
    setHeaderText(product.header_text);
    setHeaderTextAr(product.header_text_ar);
    setDescOne(product.desc_text_one);
    setDescOneAr(product.desc_text_ar_one);
    setDescTwo(product.desc_text_two);
    setDescTwoAr(product.desc_text_ar_two);
    setDescription(product.description);
    setDescriptionAr(product.description_ar);
    setVideo(product.video);
    setFooterImage(product.footer_image);
    setFooterVideo(product.footer_video);
    setFooterText(product.footer_text);
    setFooterTextAr(product.footer_text_ar);
  }, [
          ...
          <Form onSubmit={(e) => updateProductData(e)}>
            ...
            <Row>
              <FormGroup className='mb-3'>
                ...
                <FormControl
                  ...
                  disabled={isLoading}
                  onChange={() =>
                    UploadFile('editProductVideo', null, setVideo)
                  }
            ...
            <Row>
              ...
              >
                ...
                <FormControl
                  type='file'
                  id='editProductFooterImage'
                  disabled={isLoading}
                  onChange={() => {
                    UploadFile(
                      'editProductFooterImage',
                      'editProductFooterImagePreview',
                      setFooterImage
                    );
                    setFooterVideo('');
                  }}
                ...
                <img
                  src={footerImage}
                  id='editProductFooterImagePreview'
                  alt=''
              ...
              >
                ...
                <FormControl
                  type='file'
                  id='editProductFooterVideo'
                  disabled={isLoading}
                  onChange={() => {
                    UploadFile('editProductFooterVideo', null, setFooterVideo);
                    setFooterImage('');
                  }}
                  accept='video/mp4,video/x-m4v,video/*'
                />
