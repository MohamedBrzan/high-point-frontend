...
import '../../Service/Edit.css';
import {
  useGetServiceSolutionByIdQuery,
  useUpdateServiceSolutionMutation,
} from '../../../../../../store/apis/ServicesSolutions/ServicesSolutions';
...
const TabSolutionForm = ({ handleClose }) => {
  ...
  const { data: solution, isLoading } = useGetServiceSolutionByIdQuery(
    serviceSolution._id && serviceSolution._id
  );
  ...
  const [title, setTitle] = useState(serviceSolution.title || '');

  const [titleAr, setTitleAr] = useState(serviceSolution.title_ar || '');

  // Card Description

  const [description, setDescription] = useState(
    serviceSolution.description || ''
  );

  const [descriptionAr, setDescriptionAr] = useState(
    serviceSolution.description_ar || ''
  );

  // Card Images

  const [image, setImage] = useState(serviceSolution.image || '');
  ...
  const updateSolutionData = async (e) => {
    try {
      ...
      await updateSolution({ solution_id: solution._id, ...data }).then(
        (response) => {
          if (response.data) {
            dispatch(createServiceSolution(response.data));
