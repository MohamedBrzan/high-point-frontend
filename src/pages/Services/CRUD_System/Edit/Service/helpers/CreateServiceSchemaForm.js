...
import { useCreateServiceSchemaMutation } from '../../../../../../store/apis/HptServices/HptServices';
import { useTranslation } from 'react-i18next';
import '../Edit.css';
import Modal from 'react-bootstrap/Modal';
import CustomSpinner from '../../../../../../utils/CustomSpinner/CustomSpinner';

const CreateServiceSchemaForm = ({ isLoading, handleClose, refetch }) => {
  const { t } = useTranslation();
  const [createNewServiceSchema, { isLoading: creating }] =
  ...
  const createServiceSchema = async (e) => {
    try {
      e.preventDefault();
      await createNewServiceSchema({ ...data }).then((response) => {
        if (response.data) {
  ...
  return (
    <Form
      ...
      onSubmit={(e) => createServiceSchema(e)}
    >
      <Modal.Body>
        <Row>
          <Col md={6}>
