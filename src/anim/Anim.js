...
function AnimationBackground(conf) {
  conf = {
    fov: 75,
    cameraZ: 150,
    background: 0x000000,
  ...
  function init() {
    ...
    renderer.domElement.setAttribute('class', 'body_canvas');
  ...
  function updateColors() {
    ...
    for (let i = 0; i < objects.length; i++) {
      objects[i].material.color = new THREE.Color(
        cscale(TMath.randFloat(0, 1)).hex()
      );
    }
  ...
  function updateCScale(color) {
    const colors = [
      color
        ...
        .set('hsl.l', TMath.randFloat(0, 0.3))
        .hex(),
      color
        .set('hsl.s', TMath.randFloat(0, 1))
